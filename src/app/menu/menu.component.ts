...
//		if(!GlobalConstantes.m_url_bd.endsWith('/'))GlobalConstantes.m_url_bd+='/';
		GlobalConstantes.m_serveur_bd="AccesBdPm";
		this.ClasseCompteur();
...
//console.log('menu.constructeur: url_ecran='+url_ecran);
			if(url_ecran==null)
			{
...
//console.log('Menu:ValiderUtilisateur(): boucle: compteur='+GlobalConstantes.m_compteur);
			...
			{
//console.log('Menu:ValiderUtilisateur(): compteur='+GlobalConstantes.m_compteur+', id_prs='+GlobalConstantes.m_id_prs_login);
				this.onDeConnexion();
...
//					GlobalConstantes.m_id_prs_login=id_prs;
					...
					GlobalConstantes.m_compteur=0;
//console.log('MenuComponent: memorisation de m_id_prs='+GlobalConstantes.m_id_prs_login);
					this.m_phase_connexion=false;
...
//console.log('MenuComponent: memorisation de m_id_prs='+GlobalConstantes.m_id_prs);
	...
	public static NumEcran(nom_ecran:string):string
	{
		...
		var i:number;
//		var	tab_bloc_ref:string[]=['cerem','deg','etat_prs','obed','orient','rite','temple','terr','type_doc','type_fic','type_loge','type_off','type_tenue','ville'];
		switch (nom_ecran)
		{
			case 'loge':
				num_ecran='2|1';
...
//console.log('MenuComponent.AppelerHref: url='+url);
	...
	private static tab_ecrans:string[]=['references','dependances','loges','prs'];
	private static tab_bloc_ref:string[]=['cerem','deg','etat_prs','obed','orient','rite','temple','terr','type_doc','type_fic','type_loge','type_off','type_tenue','ville'];
	UrlEcran(num_ecran:number,num_bloc:number):string
	{
		var url_ecran:string=MenuComponent.tab_ecrans[num_ecran];
		if(num_ecran==0)url_ecran+='/'+MenuComponent.tab_bloc_ref[num_bloc];
		return url_ecran;
	}
	...
	/*
	public static NomTableRef(num_bloc:int):string
	{
		return MenuComponent.tab_bloc_ref[num_bloc);
	}
	*/
	ClasseCompteur()
	{
